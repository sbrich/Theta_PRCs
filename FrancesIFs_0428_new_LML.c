...
//    number=atof(argv[4]);
    ...
    double** params;
    params=(double**) malloc((74)*sizeof(double*));
    for (a=0; a<74; a++)
    {
        params[a]=(double*) malloc(4*sizeof(double));
    }
    
    FILE *fp = fopen("LML_model_params.csv", "r");
    char buf[1024];
    char *eptr;
    int row_count = 0;
    int field_count = 0;
    ...
    for (int paramnum=0; paramnum<74;paramnum++){
        
        char f1[100], f2[100], f3[100], f4[100], f5[100];
        
        int Iapptemp=Iapp;
//        sprintf(f1, "InhibitoryNetwork_%d_TrackVariables_LML.csv", paramnum);
//        sprintf(f2, "InhibitoryNetwork_%d_SpikeTimes_LML.csv", paramnum);
//        sprintf(f3, "InhibitoryNetwork_%d_InputCurrents_LML.csv", paramnum);
        sprintf(f4, "InhibitoryNetwork_PRC_LML_%d_%d.csv", paramnum, Iapptemp);
        sprintf(f5, "InhibitoryNetwork_Parameters_LML_%d_%d.csv", paramnum, Iapptemp);
