...
//    number=atof(argv[4]);
    ...
    double** params;
    params=(double**) malloc((556)*sizeof(double*));
    for (a=0; a<556; a++)
    {
        params[a]=(double*) malloc(4*sizeof(double));
    }
    
    FILE *fp = fopen("HML_model_params.csv", "r");
    char buf[1024];
    char *eptr;
    int row_count = 0;
    int field_count = 0;
    ...
    for (int paramnum=0; paramnum<556;paramnum++){
        
        char f1[100], f2[100], f3[100], f4[100], f5[100];
        
        int Iapptemp=Iapp;
//        sprintf(f1, "InhibitoryNetwork_%d_TrackVariables_HML.csv", paramnum);
//        sprintf(f2, "InhibitoryNetwork_%d_SpikeTimes_HML.csv", paramnum);
//        sprintf(f3, "InhibitoryNetwork_%d_InputCurrents_HML.csv", paramnum);
        sprintf(f4, "InhibitoryNetwork_PRC_HML_%d_%d.csv", paramnum, Iapptemp);
        sprintf(f5, "InhibitoryNetwork_Parameters_HML_%d_%d.csv", paramnum, Iapptemp);
