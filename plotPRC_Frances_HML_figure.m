...
for i=0:repnum-1
    ...
    str4=sprintf('InhibitoryNetwork_PRC_HML_%d_%d.csv', i, Iapp);
    str5=sprintf('InhibitoryNetwork_Parameters_HML_%d_%d.csv', i, Iapp);
...
str=sprintf('HML, Iapp=%d pA', Iapp);
...
filename=sprintf('PRCs_figure_HML_Iapp%d_gsyn%1.0f.fig', Iapp, variables(1,1));
savefig(filename);
filename2=sprintf('PRCs_figure_HML_Iapp%d_gsyn%1.0f.png', Iapp, variables(1,1));
...
% str=sprintf('HML, Iapp=%d pA', Iapp);
% title(str, 'FontSize', 26);
% filename=sprintf('Freqs_HML_Iapp%d_gsyn%1.0f.fig', Iapp, variables(1,1));
% savefig(filename);
